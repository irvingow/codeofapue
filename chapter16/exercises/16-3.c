...
void serve_many(int sockfd[], int nsockets)
{
    int clfd, maxsock = -1;
    FILE *fp;
    char buf[BUFLEN];
    ...
    for(;;)
    {
        ...
        for(int i = 0; i < nsockets; ++i)
        {
            ...
            set_cloexec(clfd);
            if((fp = popen("/usr/bin/uptime", "r")) == NULL)/* send error message */
            {
                sprintf(buf, "error: %s\n", strerror(errno));
            ...
            {
                while(fgets(buf, BUFLEN, fp) != NULL)/* read message from pipe and send it to client */
                    send(clfd, buf, strlen(buf), 0);
                pclose(fp);
            }
            close(clfd);
